...
fn workspace_root_slot() -> &'static RwLock<Option<PathBuf>> {
    ...
    ROOT.get_or_init(|| {
        RwLock::new(dirs::home_dir().map(|home| home.join(".insulator").join("projects")))
    })
}

pub fn set_workspace_root(root: Option<PathBuf>) {
    insulator_protocol::projectless::set_workspace_root(root.clone());
...
/// Existing builds created dated workspaces directly under `~/.insulator`; keep
...
pub fn create_workspace(prompt: Option<&str>) -> io::Result<Workspace> {
    let root = workspace_root().ok_or_else(|| {
        io::Error::new(
            ...
            "could not locate the home directory for ~/.insulator/projects",
        )
    })?;
