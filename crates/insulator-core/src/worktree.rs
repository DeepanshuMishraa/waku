...
//! worktree beneath `~/.insulator/worktrees`, then every workspace consumer uses
...
pub fn create(
    ...
    let root = dirs::home_dir()
        .ok_or_else(|| anyhow!("could not locate the home directory for ~/.insulator/worktrees"))?
        .join(".insulator/worktrees");
    create_in(
        project_path,
        &root,
        project_id,
