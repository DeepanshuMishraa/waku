...
fn repair_common_bare_latex(source: &str) -> String {
    static LIMIT: LazyLock<Regex> = LazyLock::new(|| {
        Regex::new(r"\\lim_([A-Za-z])\\to\s*([A-Za-z0-9]+)").expect("valid limit shorthand")
    });
    static SLASH_FRACTION: LazyLock<Regex> = LazyLock::new(|| {
        Regex::new(r"/\s*([^/]+?)\s*/\s*([A-Za-z0-9²]+)\s*$")
            .expect("valid slash fraction shorthand")
    });
    let repaired = LIMIT.replace_all(source, r"\lim_{$1\to $2}");
    let repaired = repaired.replace("d / dx", r"\frac{d}{dx}");
    SLASH_FRACTION
        .replace(&repaired, r"\frac{$1}{$2}")
        .into_owned()
}
...
fn typeset(latex: &Arc<str>, display: bool) -> anyhow::Result<Arc<DisplayList>> {
    ...
    let repaired = repair_common_bare_latex(latex);
    let ast = ratex_parser::parse(&repaired).map_err(|error| anyhow::anyhow!("{error:?}"))?;
