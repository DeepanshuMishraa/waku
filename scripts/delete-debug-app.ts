...
const debugBundleIdentifiers = ["sh.insulator.dev", "codes.insulator.dev"];
...
function isDebugDiagnostic(name: string): boolean {
  return /^Insulator Debug(?: Computer Use)?[-_.]/.test(name);
}
...
addCandidate(join(projectRoot, ".insulator-cache", "computer-use", "debug"));
addCandidate(join(projectRoot, "target", "debug", "Insulator Debug.app"));
...
addCandidate(join(userHome, "Applications", "Insulator Debug.app"));
addCandidate("/Applications/Insulator Debug.app");

// Debug-only app data. The release app uses Insulator/sh.insulator and is not included.
addCandidate(join(library, "Application Support", "Insulator Debug"));
addCandidate(
  join(
    library,
    "Application Support",
    "Insulator",
    "Computer Use",
    "Insulator Debug Computer Use.app",
  ),
);
addCandidate(join(library, "Caches", "Insulator Debug"));
addCandidate(join(library, "Logs", "Insulator Debug"));

// codes.insulator.dev was Insulator Debug's bundle ID before sh.insulator.dev.
...
if (targets.length === 0) {
  console.log("No Insulator Debug files or directories found.");
  process.exit(0);
}

console.log(
  "The following Insulator Debug paths, including directory contents, will be permanently deleted:\n",
);
for (const target of targets) {
  console.log(`  [${target.kind}] ${target.path}`);
}

const runningProcesses = ["Insulator Debug", "Insulator Debug Computer Use"].filter(
  (name) =>
