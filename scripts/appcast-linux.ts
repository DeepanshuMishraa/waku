...
// Sparkle-format feeds. Insulator's native Linux updater reads this same compact
...
<rss xmlns:sparkle="http://www.andymatuschak.org/xml-namespaces/sparkle" version="2.0">
  <channel>
    <title>Insulator (Linux ${arch})</title>
${entries}
  </channel>
</rss>
`;
...
export async function generateLinuxAppcasts(
  ...
  for (const arch of architectures) {
    const archive = `insulator-${version}-${targetTriple(arch)}.tar.gz`;
  ...
  if (written.length === 0) {
    throw new Error(`No insulator-${version}-<target>.tar.gz found in ${assetsDir}`);
  }
  return written;
}
