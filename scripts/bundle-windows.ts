...
// the archive half and resources/windows/insulator.iss for the installer half.
//
// Usage:
...
const packageDirectoryName = `insulator-${version}-${targetTriple}`;
const archive = join(releaseDirectory, `${packageDirectoryName}.zip`);
const installer = join(
  releaseDirectory,
  `Insulator-${version}-${architecture}-Setup.exe`,
);

await $`cargo build --locked --release --package insulator --bin insulator --package insulator-daemon --bin insulator-daemon`;

const staging = await mkdtemp(join(tmpdir(), "insulator-bundle-"));
try {
  ...
  for (const file of ["insulator.exe", "insulator-daemon.exe"]) {
  ...
  if (certificateData && certificatePassword) {
    ...
    await sign(signtool, certificate, certificatePassword, [
      join(packageDirectory, "insulator.exe"),
      join(packageDirectory, "insulator-daemon.exe"),
    ]);
  } else {
  ...
  await $`${findInnoSetupCompiler()} ${`/DAppVersion=${version}`} ${`/DArch=${architecture}`} ${`/DStageDir=${packageDirectory}`} ${`/DOutputDir=${releaseDirectory}`} ${join(projectRoot, "resources", "windows", "insulator.iss")}`;
