...
//
// <assets-dir> holds this release's `Insulator-<version>-<arch>-Setup.exe` files.
...
<rss xmlns:sparkle="http://www.andymatuschak.org/xml-namespaces/sparkle" version="2.0">
  <channel>
    <title>Insulator (Windows ${arch})</title>
${entries}
  </channel>
</rss>
`;
...
export async function generateWindowsAppcasts(
  ...
  for (const arch of architectures) {
    const installer = `Insulator-${version}-${arch}-Setup.exe`;
  ...
  if (written.length === 0) {
    throw new Error(`No Insulator-${version}-<arch>-Setup.exe found in ${assetsDir}`);
  }
  return written;
}
