# Insulator on Windows

## Install

Download `Insulator-<version>-x86_64-Setup.exe` (or the `aarch64` installer on an
Arm device) from [releases.insulator.sh](https://releases.insulator.sh) or the
[GitHub release](https://github.com/egoist/insulator/releases) and run it. It
installs per-user into `%LOCALAPPDATA%\Programs\Insulator`, so it never asks for
administrator rights — which is also what lets Insulator update itself later
without a UAC prompt.

`https://releases.insulator.sh/latest-windows.txt` names the current version if you
want to script the download.

### Portable

`insulator-<version>-<target>.zip` is the same build without an installer. Unpack it
anywhere and run `insulator.exe`.

**Keep the two executables together.** Insulator launches `insulator-daemon.exe` from its
own directory, so moving `insulator.exe` out on its own leaves it unable to start
...
Nothing else: Insulator links the C runtime statically, so there is no Visual C++
...
Insulator updates itself. It checks once per launch, and an available update
appears in the sidebar footer; clicking it downloads the installer, verifies
its signature, and runs it. Insulator closes, is replaced in place, and reopens.
...
- `https://releases.insulator.sh/appcast-windows-x86_64.xml`
- `https://releases.insulator.sh/appcast-windows-aarch64.xml`

Every installer carries an EdDSA signature, and Insulator refuses one that does not
...
`%LOCALAPPDATA%\Insulator\updater.json`.

## Where Insulator keeps its data

| What | Path |
| --- | --- |
| Tasks, sessions, transcripts | `%LOCALAPPDATA%\Insulator\app.db` |
| Attachments and blobs | `%LOCALAPPDATA%\Insulator\blobs` |
| Settings | `%USERPROFILE%\.insulator\app.json` |

Unpacking a new release over the old directory leaves all of it untouched.

## Agent CLIs

Insulator detects the provider CLIs on `PATH` and, because a fresh `PATH` may
...
Insulator hosts it in *visual* mode rather than as a child window: the page renders
...
- **Pen, touch and dragging files into the page are not wired up.** Visual
  hosting delivers no input of its own; Insulator forwards mouse, wheel, cursor and
...
**The window opens black, or the app exits at startup.** Insulator needs a working
...
shim on `PATH`. If the shell finds it but Insulator does not, set the binary path in
**Settings → Providers** and file an issue with the install method.

**Git-backed features do nothing.** Insulator shells out to `git`. Install Git for
Windows and make sure `git --version` works in a new terminal.

**The update never arrives.** Insulator reaches the feed with the `curl.exe` in
System32; a proxy or filter that blocks `releases.insulator.sh` blocks updates too.
